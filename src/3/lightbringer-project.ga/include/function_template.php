...
class Template
{
  ...
  function Template($filename)
  {
    ...
    $this->parse($this->structure);   /** JEREMY, I removed the & symbol here, it was  $this->parse(&$this->structure); **/
    $this->content = array();
  ...
  {
    while (preg_match("|<!--\ if\(([^),]+)(,([^)]+))?\)\ -->|si", $structure["body"], $m)) {
      $name = $m[1];
      $sort = $m[3];
      ...
      $block["blocks"] = array();
      $block["used"] = 0;
      $reg = "|<!--\ if\(".$name."(,[^)]*)?\)\ -->(.*)<!--\ endif\(".$name."\)\ -->|si";
      if (!preg_match($reg, $structure["body"], $m)) {
	$this->error("block `".$name."' does not have if() AND endif().");
      }
      $reg2 = "|<!--\ if\(".$name."(,[^)]*)?\)\ -->(.*)<!--\ else\(".$name."\)\ -->(.*)<!--\ endif\(".$name."\)\ -->|si";
	...
	$structure["body"] = preg_replace($reg, "{block:".$name."}", $structure["body"]);
      ...
      $this->parse($block);  /** JEREMY, I removed the & symbol here, it was  $this->parse(&$block); **/
  ...
  {
    print "<ul>";
    $this->_showContent($this->content);  /** JEREMY, I removed the & symbol here, it was  $this->_showContent(&$this->content); **/
    print "</ul>";
  }
  ...
  {
    print "<ul>";
    $this->_showStructure($this->structure);  /** JEREMY, I removed the & symbol here, it was  $this->_showStructure($this->structure); **/
    print "</ul>";
  }
  ...
  function toString()
  {
    return preg_replace(array("|&#x7b;|", "|&#x7d;|", "|&#x3a;|", "|&#x7c;|"),
			array("{", "}", ":", "|"),
			$this->_toString($this->structure, $this->content, 0));  /** JEREMY, I removed the & symbol here, it was  $this->_toString(&$this->structure, &$this->content, 0)); **/
  }
  ...
  function _toString(&$structure, &$content, $count, $max = -1)
  {
	...
	if ($block["sort"] != "" && count($content["blocks"][$blockname]) > 0) {
  	...
  	$rep = "";
	if (count($content["blocks"][$blockname]) > 0) {
  ...
  {
    $this->_gotoNext($this->content, $variable);  /** JEREMY, I removed the & symbol here, it was  $this->_gotoNext(&$this->content, $variable); **/
  }
  ...
  function _gotoNext(&$content, $variable)
  {
	...
	$content["blocks"][$parent][0] = array();
      ...
      $i = count($content["blocks"][$parent])-1;
      $this->_gotoNext($content["blocks"][$parent][$i], $var);  /** JEREMY, I removed the & symbol here, it was  $this->_gotoNext(&$content["blocks"][$parent][$i], $var); **/
    } else {
  ...
  function loadfile($filename)
  {
	...
	$content = substr($content, 15); //strip out php exit
    fclose($fd);
    return $filename2[1].$content;
  }

<?php
	...
	while ( $aRow = $rResult->fetch() )
	{
		...
		$row[3] = $aRow['views'];
		$row[4] = $aRow['added'];
		$row[5] = '<a href="index.php?page=user-preview&uid='.$aRow['author'].'">' . $aRow['author_str'] . '</a> [' . $aRow['author'] . ']';
		$row[6] = '<span class="button-group"><a href="index.php?page=edit-article&id='.$aRow['id'].'" class="button icon edit">Edit</a><a href="execute.php?take=delete&action=article&id='.$aRow['id'].'" onclick="return deletecheck(\'Are you sure you want to delete this article?\');" class="button icon remove danger">Remove</a></span>';

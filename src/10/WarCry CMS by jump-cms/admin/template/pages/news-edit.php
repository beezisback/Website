...
//Get errors
if ($error = $ERRORS->DoPrint('editNews'))
{
	echo $error;
}
unset($error);
...
if (!$id)
{
	$CORE->ErrorBox('The news id is missing.');
}
...
if ($res->rowCount() == 0)
{
	$CORE->ErrorBox('The news record is invalid or missing.');
}
...
<section id="content">
    ...
    <div class="tab" id="maintab">
        ...
        <div class="form">
            ...
            <form method="post" action="<?php echo $config['BaseURL']; ?>/admin/execute.php?take=editNews&id=<?php echo $row['id']; ?>" id="newsForm" name="editNewsForm">
                ...
                <section>
                  ...
                  <div>
                    <input id="label" name="title" type="text" class="required" value="<?php echo $row['title']; ?>"/>
                  </div>
                </section>
                ...
                <section>
                  ...
                  <div>
                    <textarea class="required" id="textarea" rows="7" name="shortText"><?php echo $row['shortText']; ?></textarea>
                  </div>
                </section>
                ...
                <section>
                  ...
                  <div>
                    <textarea class="required bbcode" id="textarea" name="text"><?php echo $row['text']; ?></textarea>
                  </div>
                </section>
                ...
                {
                    productUpdatePreview('<?php echo $config['BaseURL'] . '/uploads/news/icons/' . $row['image']; ?>', '<?php echo $row['image']; ?>');
                });
                </script>
            ...
            <section>
              ...
              <div>
                    ...
                    <form id="croppingForm" method="POST" onsubmit="return false;" action="<?php echo $config['BaseURL']; ?>/admin/execute.php?take=cropImage" name="cropForm">
                        ...
                        <input type="hidden" name="resize" value="200" />
                        ...
                        <div id="image_CroppingSection" style="display: none; margin-bottom: 5px;">
                            <div id="uploadedImagePreview" style="width: 200px !important; display: inline-block;"></div>
        ...
        <p>
        	<span class="button-group">
                ...
                <a href="index.php?page=news" class="button">Cancel</a>
         	</span>
        </p>
...
<script type="text/javascript">
	...
	{
		<?php
		if ($formData = $ERRORS->multipleError_accessFormData('editNews'))
		{	
			echo '

...
class server_RealmStats
{
	...
	public function prepareUptimeRow()
	{
		...
		$res = $AUTH_DB->prepare("SELECT starttime, uptime FROM `uptime` ORDER BY starttime DESC LIMIT 1;");
	...
	//{
		...
		//$res = $WORLD_DB->prepare("SELECT starttime, uptime FROM `uptime` WHERE `realmid` = :id ORDER BY starttime DESC LIMIT 1;");
		//$res->bindParam(':id', $this->realm, PDO::PARAM_INT);
		//$res->execute();
	...
	public function getUptimeRow()
	{
		return $this->uptimeRow;
	}
	...
	public function GetRealmDetails()
	{
		...
		//Check the cache
		if (($row = $CACHE->get('realm_details/details_' . $this->realm)) === false)
		{
			...
			// Cache for 10 min
			$CACHE->store('realm_details/details_' . $this->realm, $row, "600");
		}

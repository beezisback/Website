				...
				WarcryQueue('ARMORSETS_'+$armorsetId).add(function()
				{
					...
					$.ajax({
						...
						success: function(data)
						{
							...
							if (quality && icon) {
								$('#armor-set-'+$armorsetId+'-items').append('<a class="'+quality.toLowerCase()+'" href="http://cata.openwow.com/item='+$entry+'" rel="item='+$entry+'" style="background-image:url(http://wow.zamimg.com/images/wow/icons/medium/'+icon.toLowerCase()+'.jpg);" onclick="return false;"></a>');
							}
							else {
								$('#armor-set-'+$armorsetId+'-items').append('<a href="http://cata.openwow.com/item='+$entry+'" rel="item='+$entry+'" target="_blank"></a>');
							}

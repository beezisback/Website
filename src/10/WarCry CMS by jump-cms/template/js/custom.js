...
function updateRealmStatus(id)
{
	...
	$.get(
		...
		function(data)
		{
			...
			var uptime = new Date().getTime() - new Date(parseInt(data.uptime.starttime) * 1000).getTime();
			realmUptimes[$realm] = uptime;
			setInterval(function() { UpdateRealmUptime($realm); }, 1000);
...
function UpdateRealmUptime(realm)
{
	realmUptimes[realm] = realmUptimes[realm] + 1000;

	var uptime = realmUptimes[realm];
	var uptimeEle = $('#realm-uptime-' + realm);
	var string = '';
	var days = Math.floor(uptime / 86400000);
	var hours = Math.floor((uptime - days*86400000)/3600000);
	var minutes = Math.floor((uptime - days*86400000 - hours*3600000)/60000);
	var seconds = Math.floor((uptime - days*86400000 - hours*3600000 - minutes*60000)/1000);
	
	if (days <= 0 && hours <= 0 && minutes <= 0)
	{
		string = seconds.toString() + 's';
	}
	else if (days <= 0 && hours <= 0)
	{
		string = minutes.toString() + 'm ' + seconds.toString() + 's';
	}
	else if (days <= 0)
	{
		string = hours.toString() + 'h ' + minutes.toString() + 'm ' + seconds.toString() + 's';
	}
	else
	{
		string = days.toString() + 'd ' + hours.toString() + 'h ' + minutes.toString() + 'm ' + seconds.toString() + 's';
	}
	
	uptimeEle.html(string + ' uptime');
}

	...
	var methods =
	{
		...
		init : function(options)
		{
			...
			$('#store-search-form').on('submit', function()
			{
				...
				var selected = $('#store-class-select').find('option:selected');
				if (selected.length > 0)
					$class = selected.val();
												
				$this.WarcryStore('applyFilter', { search: $search, quality: $quality, class: $class });
		...
		loadItem : function(options)
		{
			...
			$.ajax({
				url: "ajax.php?phase=1&entry=" + $entry,
				dataType: 'json',
				success: function(data)
				{
					var name = data.name_enus;
			...
			var add = function(data)
			{
				...
				cartElement.append(
					'<ul class="selected-item-row" id="cart-item-'+ $cartItemId +'" style="display: none;">' +
                    	'<li class="sitem-icon" id="'+ $quality +'"><a href="http://cata.cavernoftime.com/item='+ $entry +'" rel="item='+ $entry +'"><span style="background-image:url(\'http://wow.zamimg.com/images/wow/icons/large/'+ $icon +'.jpg\')"></span></a></li>' +

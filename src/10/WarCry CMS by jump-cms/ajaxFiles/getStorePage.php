...
$res = $DB->prepare("SELECT id, entry, realm, name, Quality, gold, silver FROM `store_items` ".$where." ORDER BY entry DESC LIMIT ".$offset.",".$perPage);
	...
	while ($arr = $res->fetch())
	{
		...
		<item id="', $arr['id'], '">
			...
			<html>  
				<![CDATA[
				<li id="store-item-', $i, '">
					<div class="common" id="item-cont">
					
						<div class="item-ico">
							<a href="', $config['WoWDB_URL'], '/item=', $arr['entry'], '" class="item-ico ', strtolower($CORE->getItemQualityString($arr['Quality'])), '" rel="item=', $arr['entry'], '" id="icon">
							</a>
						</div>
						<div class="item-info">
			           		<p>', $arr['name'], '</p>
			           		<span id="info"><br>', $arr['name'], '</span>

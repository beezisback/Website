...
$res = $DB->prepare("SELECT id, entry, name, Quality, realm, gold, silver FROM `store_items` ".$where." ORDER BY entry DESC LIMIT ".$offset.",".$perPage);
	...
	while ($arr = $res->fetch())
	{
		...
		<item id="', $arr['id'], '">
			...
			<html>  
				...
				<li id="store-item-', $i, '">
					<div id="item-cont">
					<a href="', $config['WoWDB_URL'], '/?item=', $arr['entry'], '" rel="item=', $arr['entry'], '" class="item-ico ', strtolower($CORE->getItemQualityString($arr['Quality'])), '" style="background-image:url(http://wow.zamimg.com/images/wow/icons/large/', strtolower($data['icon']), '.jpg);"></a>

...
function unstuck($guid)//returns
{
	...
	$fail=false;
	$sql="UPDATE ".$db_translation['characters']." INNER JOIN character_homebind
ON ".$db_translation['characters'].".guid = character_homebind.guid AND  ".$db_translation['characters'].".guid = '".$guid."'
...
".$db_translation['characters'].".position_z = character_homebind.position_z";
	//(c)AXE. Feel free to use this SQL script to unstuck using MaNGOS,
	// except if you are associated in any way with pinguecms, then you are forbidden to use this.
	$db->query($sql) or ($fail=mysql_error());
	return $fail;
}
...
function teleport($guid,$x,$y,$z,$map,$newGold)//returns
{
	global $db,$db_translation;
	$fail=false;
	$db->query( "UPDATE ".$db_translation['characters']." SET position_X = ".$x.", position_Y = ".$y.", position_Z = ".$z.", map = ".$map.", ".$db_translation['characters_gold']." = ".$newGold." WHERE guid = '".$guid."'" ) or ($fail=mysql_error());	
	return $fail;
}
/***************************************
...
function create_account($user,$pass,$email)//gotta be logged in for this, returns
{
	...
	$db->query("INSERT INTO ".$db_translation['accounts']." (".$db_translation['login'].",".$db_translation['encrypted_password'].",".$db_translation['gm'].",".$db_translation['banned'].",".$db_translation['email'].",".$db_translation['flags'].") VALUES ('".strtoupper($user)."','".$pass_enc."','0','0','".$db->escape($email)."','".$db_translation['expansion_wotlk']."')") or ($report = 'Error: '.mysql_error());
	if (!$report)
		return false;
	else
		return $report;

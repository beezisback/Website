...
if (!isset($_SESSION['realm'])) 
{
	...
	foreach ($realm as $id => $data)
	{
		$cont2.="<td><form method='POST' action='./quest_ac.php?name=Donation_Shop'><input type='hidden' value='".$id."' name='id'><div id='log-b2'><input type='submit' value='".$data['name']."' name='realm' /></div></form></td>";
	}
...
if (isset($_POST['action']))
{
	...
	$res = $WEB_PDO->prepare("SELECT * FROM `shop` WHERE `id` = :itemsgrp AND donateorvote='1' LIMIT 1");
	...
	if (isset($realm[$realm_data123]))
	{
		...
		$time = date("m-d-Y, h:i");
		$refnum = date("jnGis");
		$subject = 'WebsiteDonationShopREF'.$refnum.'';
		...
		if (substr($sendingmail, 0, 16)=="<!-- success -->")
		{
			...
			$update = $WEB_PDO->prepare("UPDATE `accounts_more` SET `dp` = '".$newpoints."' WHERE `acc_login` = :acc");
			...
			$ins = $WEB_PDO->prepare("INSERT INTO `donation_log` (user,cost, date,account,itemid) VALUES (:char,'$cost', '$time', :acc,'$itemid')");
...
{ 
 	if ($_POST['additem']) 
 	{
		if ($_POST['sep']=='0') /*is item*/  
 		{
			if ($_POST['itemid']=='')   
 			...
 			{
				$insert = $WEB_PDO->prepare("INSERT INTO `shop` (sep, name, itemid, color, cat, sort, cost, charges, donateorvote, description, custom, realm) VALUES ('0', :name, :iid, :color, :cat, :sort, :points, :charges,'1', :descr, :custom, :realm)");
		...
		{    
			...
			{
				$insert = $WEB_PDO->prepare("INSERT INTO shop (sep, name, cat, sort, donateorvote, itemid) VALUES ('1', :name, :cat, :sort, '1', '0')");
				$insert->bindParam(':name', $_POST['name'], PDO::PARAM_STR);
				$insert->bindParam(':cat', $_POST['cat'], PDO::PARAM_STR);
				$insert->bindParam(':sort', $_POST['sort'], PDO::PARAM_STR);
...
$cont2.="</table>";
$cont2.='<div align="left"><div class="small_box_1" style="width: 160px;"> 
You have '. $a_user['dp'].' Donation Points.
</div></div> <br/>
<form method="post" action="">
<table border="0" width="700" align="center" cellpadding="3" cellspacing="0" class="ShopTable">    
 	<tr>    
 		<td>Item Name</td>
		<td>Charges</td> 
 		<td>Description</td>     
...
$res = $WEB_PDO->prepare("SELECT * FROM `shop` WHERE `donateorvote` = '1' AND `realm` = :realm OR `donateorvote` = '1' AND `realm` = '0' ORDER BY cat, sort ASC");       
	...
	/*its seperator*/    
 	...
 	{           
		$cont2.= "<tr><td colspan='5'>";    
 		if ($a_user[$db_translation['gm']]==$db_translation['az'])     
 		{
			$cont2.= '<a href="./quest.php?name=Donation_Shop&points=1&delid='.$items['id'].'">[x]</a> ';    
 		}
		...
		if ($items['custom']=='1')        
 		{
			/*color codes here*/         
 			...
 			$cont2.= '<span style="color:'.$cil[$items['color']].'" onmouseover="$WowheadPower.showTooltip(event, \'<font color='.$cil[$items['color']].'>'.$items['name'].'</font><br><small>This is a donation token.</small>\')" onmousemove="$WowheadPower.moveTooltip(event)" onmouseout="$WowheadPower.hideTooltip();">['.$items['name'].']</span></td>';
		}
		else
		{
...
$cont2.='
 ...
 <div class="new_vote_searchdiv" align="center">Select Your Chracter: <select name="character">'; 
...
$cont2.= '
		...
		Please make sure that you have carefully read all relevant documentation and tried out the website services. All our sales are final and no refunds will be given. As all of our Customized Items in game are nontangible, delivered instantly once payment is received it is impossible for you to return a Customized Item to use for a refund.
	</div>
</form>
</div>
</center>';        

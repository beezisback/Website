...
{
	$cont1.=  '&nbsp;&nbsp;<a href="./honor.php?realm='.$id.'">'.$data['name'].'</a>&nbsp;&nbsp;';
	if ($realmid == $id)
	{
		$rname = $data['name'];
	}
...
if ($REALM_DB)
{
	...
	{
		$res = $REALM_DB->prepare("SELECT * FROM `".$db_translation['characters']."` WHERE `".$db_translation['characters_honorPoints']."` <> '0' ORDER BY ".$db_translation['characters_honorPoints']." DESC LIMIT 20");
		$res->execute();

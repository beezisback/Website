		...
		if(!empty($_POST['username']) && !empty($_POST['email']) && !empty($_POST['password']) && !empty($_POST['re-password']))
		{
			...
			if(ValidateUsername($username) && ValidateEmail($email))
			{
				...
				if($data->fetchColumn() == 0)
				{
					...
					{
						echo '<div class="callout alert">Капча не пройдена!</div>';
					}
				}
				else
				{
					echo '<div class="callout alert">Логин или почта уже заняты!<div>';
				}
			}
			else
			{
				echo '<div class="callout alert">Логин или почта некорректны!</div>';
			}
		}
		else
		{
			echo '<div class="callout alert">Поля должны быть заполнены!<div>';
		}
	}
}

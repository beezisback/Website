...
<?php
include "config.php";
mysql_select_db("authhell") or die(mysql_error());
...
echo '<a class="q'.$row['Quality'].'" style="text-decoration:none; cursor: default;" href="http://tbc.cavernoftime.com/item='.$row['entry'].'" onclick="return false"><spam onClick="fill(\''.$row['name'].'\');">'.$row['name'].'</spam></a>';
echo'</div>';                               
}
}
}

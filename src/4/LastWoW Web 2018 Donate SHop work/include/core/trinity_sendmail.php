...
function sendmail($playername,$playerguid, $subject, $text, $item, $shopid=0, $money=0, $realmid=false) //returns, IMPORTANT: do not remove <!-- success --> if success
{
	...
	if ($money=='')
	   ...
	   $query_string ='INSERT INTO mail_external(receiver, subject, message, item, item_count) VALUES("'.$playerguid.'", "'.$subject.'", "'.$text.'", "'.$item.'", "1")';
	...
	if($sendmail==false && $sql2[0]<>'')
		$db->query("INSERT INTO mail_items (mail_id,item_guid,item_template,receiver) VALUES ('".$sql2[0]."','".$item."','".$item."','".$playerguid."')") or ($sendmail.=mysql_error());
	if($sendmail==false)
		return  "<!-- success --><span class=\"colorgood\">Mail is sent! <br>All done!</span>";
	else
		return  "<span class=\"colorbad\">Mail is not sent! Error returned: ".$sendmail."<br>";
...
function sendmail_secondpatch($playername,$playerguid, $subject, $text, $item, $shopid=0, $money=0, $realmid=false) //returns, IMPORTANT: do not remove <!-- success --> if success
{
	...
	$insert_data = array(
        ...
        $query_string = strtr('INSERT INTO mail_external(receiver, subject, message, item, item_count) VALUES("%reciver%", "%subject%", "%message%", %item%, %item_count%)', $insert_data);
        $sendmail=false;
	$db->query($query_string) or ($sendmail=mysql_error());
	if($sendmail==false)
		return  "<!-- success --><span class=\"colorgood\">Mail is sent! <br>All done!</span>";

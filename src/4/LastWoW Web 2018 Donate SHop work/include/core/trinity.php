...
$db_translation = array(
		...
		"accounts" 					=> "account",	 	//table 'accounts'
			"acct"					=> "account_id",
			"login"    				=> "username",   		
			"pass_hash" 				=> "",	 	//leave blank if doesnt exits
...
function create_account($user,$pass,$email)//gotta be logged in for this, returns
{
	...
	$db->query("INSERT INTO ".$db_translation['accounts']." (".$db_translation['login'].",".$db_translation['encrypted_password'].",".$db_translation['email'].",".$db_translation['flags'].") VALUES ('".$user."','".$pass_enc."','".$db->escape($email)."','1')") or ($report = 'Error: '.mysql_error());
	if (!$report)
		return false;
	else
		return $report;

...
function create_account($user,$pass,$email)//gotta be logged in for this, returns
{
	...
	$db->query("INSERT INTO ".$db_translation['accounts']." (".$db_translation['login'].",".$db_translation['encrypted_password'].",".$db_translation['banned'].",".$db_translation['email'].",".$db_translation['flags'].") VALUES ('".$user."','".$pass_enc."','0','".$db->escape($email)."','1')") or ($report = 'Error: '.mysql_error());
	if (!$report)
		return false;
	else
		return $report;

	...
	if($telnet)
	{
		...
		//{
			if ($item<>'' && $item<>'0')//send item
			{
				...
				fputs($telnet, ".senditems ".$playername." \"".$subject."\" \"".$text."\"".$item."\n");
				$easf=time();
				$mailtext="<script type='text/javascript'> alert('A mail with an item was sent!'); </script>";
				
			}
			elseif ($money>'0' && $money<>'')//send money
			{
				fputs($telnet, ".sendmoney ".$playername." \"".$subject."\" \"".$text."\" ".$money."\n");
				$moneytext="A mail with gold was sent!";
			}
			else //send letter
			{
				fputs($telnet, ".sendmail ".$playername." \"".$subject."\" \"".$text."\"\n");
...
{	
	...
	if(mysql_num_rows($check)=='0')
	{
		...
		return "<script type='text/javascript'> alert('Item is successfully sent!'); </script>";//do not change text or 'recheck' script will not work
}

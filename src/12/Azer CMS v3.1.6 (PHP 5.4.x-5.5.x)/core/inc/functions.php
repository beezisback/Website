...
class news
{
  ...
  function news()
  {
    global $db, $db_data, $system_pg_limit;
    
    $page_data = explode("-", pgn($system_pg_limit));
      $start = $page_data[0];
        $limit = $page_data[1];
    
    $sql = $db->query("SELECT id, title, author, body, date, avatar FROM $db_data.news ORDER BY id DESC LIMIT $start, $limit");
      while($get = bbcode($db->get($sql)))
        $this->get_news[] = $get = str_replace(array("\r\n", "\r", "\n"), "<br />", $get);
        
    $sql_102 = $db->query("SELECT id FROM $db_data.news ORDER BY id DESC");
      $sql_102_check = $db->num($sql_102);
        $last_page = ceil($sql_102_check / $system_pg_limit);
        
    $nxt = "";
    
    for($next = 1; $next <= $last_page; $next++)
    {
      $nxt .= "<div class=\"page_button\" style=\"text-align:center;\"><a href=\"./?pg={$next}\">{$next}</a></div>";
    }
    
    if(isset($_GET['pg']) && $_GET['pg'] > $last_page)
    {
      header("Location: ./?pg={$last_page}");
    }
...
function register()
{
  global $db, $db_data, $db_acc, $email_validation, $server_email, $server_title, $domain, $expansion, $ip;
  ...
  if(!empty($_POST['create']))
  {
    ...
    if(empty($_POST['abot']))
    {
      return"Invalid Captcha or Equation.";
    }
    else
    {
      $abot = clean($_POST['abot']);
      
      $verify = $db->query("SELECT id, body, ipa FROM $db_data.verify WHERE BINARY body='$abot' AND ipa='$ip' ORDER BY id DESC LIMIT 1");
        $validate = npm($db->num($verify));
        
      if(isset($_POST['valid']) && $abot != "$code")
      {
        return "{$valid[0]} + {$valid[1]} doesn't equal $abot";
      }
      else if($validate != 1 && $abot != $code)
      {
...
{
  global $db_host, $soap_host, $port_host;
...
function tool_unstuck()
{
  ...
  if($login)
  {
    if(!empty($_POST['submit-unstuck']))
    {
      if(!empty($_POST['unstuck']))
      {
        $var = $_POST['unstuck'];
          $data = explode("-", $var);
...
function add_to_cart()
{
  ...
  if(!empty($_POST['atc']) || !empty($_POST['rfc']) && isset($_GET['page']) && isset($_GET['data']) && $_GET['page'] == "store_shop")
  {
    $data = $_GET['data'];
        $pos = strpos($data, "-");
        if($pos == TRUE)
...
function exe_send($command, $spt)
{
  global $soap_host, $soap_user, $soap_pass;
...
function purchase()
{
  if(isset($_GET['page']) && !empty($_POST['purchase']) && !empty($_POST['shop_data']) && $_GET['page'] == "store_shop")
  {
    ...
    $sqlr = $db->query("SELECT id, char_db, port, soap_port FROM $db_data.realms WHERE id='$d1'");
      $chkr = $db->num($sqlr);

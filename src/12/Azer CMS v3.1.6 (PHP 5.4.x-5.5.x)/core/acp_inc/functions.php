...
function acp_login()
{
  if(!isset($_SESSION['acp']) && !empty($_POST['login']))
  {
    ...
    $sql = $db->query("SELECT global_title, email_activation, login, copyright, realmlist, server_email, domain, server_title, expansion FROM $db_data.global");
      while($get = $db->get($sql))
      {
        ...
        if($get['expansion'] == 2)
        {
          $this->data['exp'] = "Wrath Of The Lich King";
            $this->data['exp1'] = "Cataclysm";
              $this->data['exp2'] = "Mists of Pandaria";
        ...
        elseif($get['expansion'] == 3)
        {
          ...
          $this->data['exp'] = "Mists of Pandaria";
            $this->data['exp1'] = "Wrath of the Lich King";
              $this->data['exp2'] = "Cataclysm";
           
          $this->data['ex'] = "4"; 
            $this->data['ex1'] = "2"; 
              $this->data['ex2'] = "3";
          
          $sqli = $db->query("UPDATE $db_acc.account SET expansion='4'"); 
        }
...
function global_update()
{
  ...
  if(isset($_GET['page']) && $_GET['page'] == "properties" && !empty($_POST['update']))
  {
...
function vote_action()
{
  ...
  if(isset($_GET['page']) && !empty($_POST['save']) && $_GET['page'] == "manage_sites")
  {
...
function account_search()
{
  ...
  {
    if(!empty($_POST['find_characters']))
    {
...
function update_account()
{
  ...
  if(isset($_GET['page']) && !empty($_POST['delete']) && $_GET['page'] == "manage_accounts_edit")
  {
    $id = clean($_POST['uid']);
  ...
  if(isset($_GET['page']) && !empty($_POST['save']) && $_GET['page'] == "manage_accounts_edit")
  {
    ...
    {
      $log = "Your account was manually deactivated/updated by [Administrator] {$admin}";
    }
...
class store_realms
{
  ...
  function store_realms()
  {
    ...
    {
      $sql = $db->query("SELECT id, rname, type, oid, char_db, port, soap_port FROM $db_data.realms ORDER BY id");
        while($get = $db->get($sql))
        {
          $this->get[] = $get;
        }
...
function store_cat_action()
{
  ...
  if(isset($_GET['page']) && !empty($_POST['save']) && $_GET['page'] == "manage_categories")
  {
...
function store_item_action()
{
  ...
  if(isset($_GET['page']) && !empty($_POST['save']) && $_GET['page'] == "manage_items")
  {
...
function realm_update()
{
  ...
  if(isset($_GET['page']) && !empty($_POST['save']) && $_GET['page'] == "manage_realms")
  {
    $rname = strip($_POST['rname']);
        ...
        $oid = clean($_POST['oid']);
          $cdb = strip($_POST['cdb']);
            $port = clean($_POST['port']);
              $soap = clean($_POST['soap']);
    
    $sql = $db->query("INSERT INTO $db_data.realms (rname, type, oid, char_db, port, soap_port) VALUES ('$rname', '$type', '$oid', '$cdb', '$port', '$soap')");
      header("Location: ./?page=manage_realms");
  }
  
  if(isset($_GET['page']) && !empty($_POST['save']) && $_GET['page'] == "manage_realms_edit")
  {
    $rname = strip($_POST['rname']);
      $type = strip($_POST['type']);
        $oid = clean($_POST['oid']);
    ...
    $sql = $db->query("UPDATE $db_data.realms SET rname='$rname', type='$type', oid='$oid', char_db='$cdb', port='$port', soap_port='$soap' WHERE id='$id'");
      header("Location: ./?page=manage_realms");
  }
...
class realms_edit
{
  ...
  function realms_edit()
  {
    ...
    {
      $id = clean($_GET['id']);
      $sql = $db->query("SELECT id, rname, type, oid, char_db, port, soap_port FROM $db_data.realms WHERE id='$id'");
        while($get = $db->get($sql))
        {
          $this->get[] = $get;
        }
